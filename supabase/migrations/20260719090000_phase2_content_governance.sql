...
begin
  ...
  if p_entity_kind = 'question' then
    ...
    if exists (
      ...
      where option_row.question_id = p_entity_id
        and option_row.enabled
        and lower(btrim(coalesce(option_row.content ->> 'text', trim(both '"' from option_row.content::text)))) in ('all of the above', 'all of the above.', 'none of the above', 'none of the above.')
    ) then
