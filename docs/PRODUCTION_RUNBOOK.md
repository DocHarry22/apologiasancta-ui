...
- `ADMIN_TOKEN` (must match engine, server only)
...
- `ADMIN_TOKEN` (high-entropy, must match UI)
...
Never expose `ADMIN_TOKEN`, `PLAYER_JOIN_SECRET`, database credentials or Supabase service-role keys through `NEXT_PUBLIC_*` variables.

## Release order

1. Review both coordinated pull requests and confirm CI.
2. Add `PLAYER_JOIN_SECRET` to Render before merging the engine PR.
3. Merge the engine PR; wait for Render `/health` to report healthy PostgreSQL persistence and check `/diagnostics` for readiness booleans only.
4. Smoke-test register, room join, answer, SSE reconnect and leaderboard with a test room.
5. Confirm Hostinger's Git integration is connected to `DocHarry22/apologiasancta-ui`, production branch `main`, Node 22+, build `npm ci --include=dev && npm run build`, start `npm start`.
6. Merge the UI PR and verify `/`, `/learn`, `/practice`, `/mobile`, `/leaderboard`, `/library` and `/admin/login`.
7. Confirm browser requests use the Render HTTPS origin and that an unapproved Origin is rejected.
